...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { timeMin, timeMax } = toSaoPauloDayRange(dateISO);
    ...
    const block = items.find(
      (e) => e?.extendedProperties?.private?.synna_block === "1" && e?.extendedProperties?.private?.slot === slotKey,
    );

    if (!block?.id) {
      return res.status(200).json({ success: true, unblocked: false, message: "No block found for this slot" });
    }
    ...
    return res.status(200).json({ success: true, unblocked: true });
  } catch (err) {
    console.error("[admin-unblock-slot]", err);
